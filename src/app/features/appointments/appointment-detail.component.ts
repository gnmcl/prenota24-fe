          ...
          <div class="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">
                  Appuntamento
                </h2>
              ...
              <p class="mt-1 text-sm text-[var(--text-secondary)]">
                {{ formatFull(apt()!.startDatetime) }} — {{ formatTime(apt()!.endDatetime) }}
              </p>
            </div>
          </div>

          <div class="mb-6">
            <app-button variant="chat" [isLoading]="openingConversation()" (click)="openConversation()">Apri conversazione</app-button>
          </div>
          ...
          @if (editMode()) {
            <app-card extraClass="mb-6 !border-indigo-200 dark:!border-indigo-800/60 !border-2">
              ...
              <div class="mb-4">
                ...
                >
                <div
                  class="rounded-xl border border-[var(--surface-card-border)] overflow-hidden max-w-sm"
                >
                  <div
                    class="flex items-center justify-between bg-[var(--surface-hover)] px-3 py-2"
                  >
                    <button
                      (click)="editCalPrevMonth()"
                      type="button"
                      class="rounded-lg p-1 text-[var(--text-secondary)] hover:bg-[var(--surface-card)] transition-colors"
                    >
                      <svg
                        class="h-3.5 w-3.5"
                        fill="none"
                  ...
                  <div
                    class="grid grid-cols-7 bg-[var(--surface-hover)] border-t border-[var(--surface-card-border)]"
                  >
                    @for (day of daysOfWeek; track day) {
                      <div
                        class="py-1.5 text-center text-[10px] font-medium text-[var(--text-tertiary)] uppercase"
                      >
                        {{ day }}
                      </div>
                    }
                  ...
                  <div class="grid grid-cols-7">
                    @for (day of editCalDaysVisible(); track $index) {
                      @if (day) {
                        <button
                          (click)="setEditDate(day.dateStr)"
                          type="button"
                          [disabled]="day.isPast"
                          [class]="
                            day.dateStr === editDate
              ...
              <div class="grid gap-4 sm:grid-cols-3 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    (ngModelChange)="setEditDuration($event)"
                    min="5"
                    step="5"
                    class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2.5 text-sm text-[var(--text-primary)] shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-colors"
                  />
              ...
              @if (editDate && editProfessionalId()) {
                <div
                  class="mb-4 rounded-xl border border-[var(--surface-card-border)] bg-[var(--surface-hover)] p-4"
                >
                  @if (editStartTime()) {
                    <div class="mb-3 flex items-center gap-2">
                      @if (editResolvedSlot()) {
                        ...
                        <span class="text-xs font-medium text-red-600 dark:text-red-400"
                          >Orario non disponibile o già occupato</span
                        >
                      }
                    </div>
                  }
                  @if (editLoadingSlots()) {
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      @for (slot of editAvailableSlots(); track slot.start) {
                        <button
                          (click)="editStartTime.set(formatTime(slot.start))"
                          type="button"
                          [class]="
                            editStartTime() === formatTime(slot.start)
              ...
              <div class="mb-4">
                ...
                <div class="max-h-36 overflow-y-auto space-y-1">
                  @for (pro of allProfessionals(); track pro.id) {
                    <button
                      (click)="selectEditProfessional(pro.id)"
                      type="button"
                      [class]="
                        editProfessionalId() === pro.id
          ...
          @if (!editMode()) {
            <app-card extraClass="mb-6">
              <dl class="divide-y divide-[var(--surface-card-border)]">
                <div class="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                  <dt
                    class="w-32 shrink-0 text-xs font-medium uppercase tracking-wider text-[var(--text-tertiary)]"
                  >
                    Cliente
                  </dt>
                  <dd class="min-w-0 flex-1">
                    <a
                      [routerLink]="['/clienti', apt()!.clientId]"
                ...
                <div class="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                  ...
                  <dd class="min-w-0 flex-1">
                    <a
                      [routerLink]="['/professionisti', apt()!.professionalId]"
                      class="font-medium text-[var(--color-primary)] hover:underline"
                    >
                ...
                @if (apt()!.notes) {
                  <div class="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
                    <dt
                      class="w-32 shrink-0 text-xs font-medium uppercase tracking-wider text-[var(--text-tertiary)] mt-0.5"
                    >
                      Note
                    </dt>
                    <dd class="text-sm text-[var(--text-secondary)] whitespace-pre-line">
                      {{ apt()!.notes }}
                    </dd>
                  </div>
                }
                @if (apt()!.cancellationReason) {
                  <div class="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
                    <dt
                      class="w-32 shrink-0 text-xs font-medium uppercase tracking-wider text-red-400 mt-0.5"
                    >
                      Cancellazione
                    </dt>
                    <dd class="flex-1">
                      ...
                      @if (apt()!.cancelledBy) {
                        <p class="mt-0.5 text-xs text-[var(--text-tertiary)]">
                          Da: {{ cancelledByLabel(apt()!.cancelledBy!) }}
                        </p>
                      }
                    </dd>
                  </div>
                ...
                @if (apt()!.proposedStart) {
                  <div class="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                    <dt
                      class="w-32 shrink-0 text-xs font-medium uppercase tracking-wider text-blue-500"
                    >
                      Orari proposti
                    </dt>
                    <dd class="text-sm font-medium text-blue-700 dark:text-blue-300">
                      <div>
                        1. {{ formatFull(apt()!.proposedStart!) }} —
                        {{ formatTime(apt()!.proposedEnd!) }}
                      </div>
                      @if (apt()!.proposedStart2) {
                        <div>
                          2. {{ formatFull(apt()!.proposedStart2!) }} —
                          {{ formatTime(apt()!.proposedEnd2!) }}
                        </div>
                      }
                      @if (apt()!.proposedStart3) {
                        <div>
                          3. {{ formatFull(apt()!.proposedStart3!) }} —
                          {{ formatTime(apt()!.proposedEnd3!) }}
                        </div>
                      }
                    </dd>
                  </div>
            ...
            @if (
              ...
              <div class="mb-6 flex flex-wrap gap-2">
                @if (apt()!.status === 'REQUESTED') {
                  <app-button (click)="confirm()" [isLoading]="actionLoading()"
                    >Conferma</app-button
                  >
                  <app-button variant="secondary" (click)="openProposeForm()"
                    >Proponi orario</app-button
                  >
                  <app-button variant="danger" (click)="cancelDialogOpen.set(true)"
                    >Rifiuta</app-button
                  >
                }
                @if (apt()!.status === 'CONFIRMED') {
                  <app-button (click)="complete()" [isLoading]="actionLoading()"
                    >Completa</app-button
                  >
                  <app-button variant="secondary" (click)="noShow()" [isLoading]="actionLoading()"
                    >Non presentato</app-button
                  >
                  <app-button variant="danger" (click)="cancelDialogOpen.set(true)"
                    >Cancella</app-button
                  >
                ...
                @if (apt()!.status === 'PROPOSED_NEW_TIME') {
                  <app-button (click)="confirm()" [isLoading]="actionLoading()"
                    >Conferma nuovo orario</app-button
                  >
                  <app-button variant="danger" (click)="cancelDialogOpen.set(true)"
                    >Cancella</app-button
                  >
            ...
            @if (showProposeForm()) {
              <app-card extraClass="!border-blue-200 dark:!border-blue-900/50">
                <h4 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">
                  Proponi nuovo orario
                </h4>
                <p class="mb-4 text-xs text-[var(--text-secondary)]">
                  Orario attuale:
                  <span class="font-semibold"
                    >{{ formatFull(apt()!.startDatetime) }} –
                    {{ formatTime(apt()!.endDatetime) }}</span
                  >
                </p>

                <!-- Proposta principale (obbligatoria) -->
                <p
                  class="mb-2 text-xs font-semibold uppercase tracking-wider text-[var(--text-tertiary)]"
                >
                  Proposta principale *
                </p>
                <div class="grid gap-4 lg:grid-cols-[240px_1fr]">
                  <div>
                    <label
                      class="mb-1 block text-xs font-semibold uppercase tracking-wider text-[var(--text-tertiary)]"
                      >Data</label
                    >
                    <input
                      type="date"
                  ...
                  <div>
                    @if (proposalSlotsLoading()) {
                      <div class="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                        <div
                          class="h-4 w-4 animate-spin rounded-full border-2 border-[var(--surface-card-border)] border-t-[var(--color-primary)]"
                        ></div>
                      ...
                      >
                      <select
                        (change)="onProposalSlotSelect($event)"
                        class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-[var(--color-primary)] focus:outline-none"
                      >
                        ...
                        @for (slot of proposalSlots(); track slot.start) {
                          <option
                            [value]="slot.start"
                            [selected]="selectedProposalSlot()?.start === slot.start"
                          >
                ...
                @if (selectedProposalSlot()) {
                  <p class="mt-2 text-xs text-green-700">
                    ✓ Proposta 1: {{ formatProposalSlot(selectedProposalSlot()!) }}
                  </p>
                }
                ...
                <div class="mt-5 border-t border-[var(--surface-card-border)] pt-4">
                  ...
                  <div class="grid gap-4 lg:grid-cols-[240px_1fr]">
                    <div>
                      ...
                      <input
                        ...
                        [value]="proposalDate2()"
                        [min]="todayStr"
                        (change)="onProposalDateChange2($event)"
                        class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-[var(--color-primary)] focus:outline-none"
                      />
                    </div>
                    <div>
                      @if (proposalSlotsLoading2()) {
                        ...
                        >
                        <select
                          (change)="onProposalSlotSelect2($event)"
                          class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-[var(--color-primary)] focus:outline-none"
                        >
                          <option value="">— Seleziona orario —</option>
                          @for (slot of proposalSlots2(); track slot.start) {
                            <option
                              [value]="slot.start"
                              [selected]="selectedProposalSlot2()?.start === slot.start"
                            >
                              {{ formatProposalSlot(slot) }}
                            </option>
                          }
                  ...
                  @if (selectedProposalSlot2()) {
                    <p class="mt-2 text-xs text-green-700">
                      ✓ Proposta 2: {{ formatProposalSlot(selectedProposalSlot2()!) }}
                    </p>
                  }
                </div>
                ...
                <div class="mt-5 border-t border-[var(--surface-card-border)] pt-4">
                  ...
                  <div class="grid gap-4 lg:grid-cols-[240px_1fr]">
                    <div>
                      ...
                      <input
                        ...
                        [value]="proposalDate3()"
                        [min]="todayStr"
                        (change)="onProposalDateChange3($event)"
                        class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-[var(--color-primary)] focus:outline-none"
                      />
                    </div>
                    <div>
                      @if (proposalSlotsLoading3()) {
                        ...
                        >
                        <select
                          (change)="onProposalSlotSelect3($event)"
                          class="w-full rounded-lg border border-[var(--surface-card-border)] bg-[var(--surface-card)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-[var(--color-primary)] focus:outline-none"
                        >
                          <option value="">— Seleziona orario —</option>
                          @for (slot of proposalSlots3(); track slot.start) {
                            <option
                              [value]="slot.start"
                              [selected]="selectedProposalSlot3()?.start === slot.start"
                            >
                              {{ formatProposalSlot(slot) }}
                            </option>
                          }
                  ...
                  @if (selectedProposalSlot3()) {
                    <p class="mt-2 text-xs text-green-700">
                      ✓ Proposta 3: {{ formatProposalSlot(selectedProposalSlot3()!) }}
                    </p>
                  }
                </div>
...
export class AppointmentDetailComponent implements OnInit {
  ...
  readonly editCalMonthLabel = computed(
    () => `${MONTHS_IT[this.editCalMonth()]} ${this.editCalYear()}`,
  );
  ...
  readonly editResolvedSlot = computed(() => {
    ...
    if (apt) {
      ...
      const origDuration = Math.round(
        (new Date(apt.endDatetime).getTime() - origStart.getTime()) / 60000,
      );
      if (
        time === origTime &&
        this.editDate === origDate &&
        this.editProfessionalId() === apt.professionalId &&
        this.editDuration() === origDuration
      ) {
  ...
  readonly nearbyProposalSlots = computed(() => {
    ...
    return [...this.proposalSlots()]
      .sort(
        (a, b) =>
          Math.abs(new Date(a.start).getTime() - target) -
          Math.abs(new Date(b.start).getTime() - target),
      )
      .slice(0, 6);
  });
  ...
  openConversation(): void {
    const appointment = this.apt();
    if (!appointment || this.openingConversation()) return;
    this.openingConversation.set(true);
    this.conversationService.openForAppointment(appointment.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: conversation => {
        this.openingConversation.set(false);
        void this.router.navigate(['/messaggi', conversation.id], { queryParams: { appuntamento: appointment.id } });
      },
      error: (error: unknown) => {
        this.openingConversation.set(false);
  ...
  private loadEditSlots(): void {
    ...
    this.editSlotTimeout = setTimeout(() => {
      ...
      this.profService.getAvailableSlots(profId, date, duration).subscribe({
        next: (slots) => {
          this.editAvailableSlots.set(slots);
          this.editLoadingSlots.set(false);
        },
  ...
  editCalPrevMonth(): void {
    if (this.editCalMonth() === 0) {
      this.editCalMonth.set(11);
      this.editCalYear.update((y) => y - 1);
    } else {
      this.editCalMonth.update((m) => m - 1);
    }
  }
  editCalNextMonth(): void {
    if (this.editCalMonth() === 11) {
      this.editCalMonth.set(0);
      this.editCalYear.update((y) => y + 1);
    } else {
      this.editCalMonth.update((m) => m + 1);
    }
      ...
      .update(a.id, {
        ...
        professionalId:
          this.editProfessionalId() !== a.professionalId
            ? (this.editProfessionalId() as import('../../core/models/domain.model').UUID)
            : undefined,
  ...
  cancelAppointment(): void {
    this.doAction(
      () => this.aptService.cancel(this.apt()!.id),
  ...
  private loadProposalSlots2(): void {
    ...
    this.profService
      .getAvailableSlots(apt.professionalId, this.proposalDate2(), durationMinutes)
      .subscribe({
        next: (slots) => {
          this.proposalSlots2.set(slots);
          this.proposalSlotsLoading2.set(false);
        },
        error: () => {
          this.proposalSlots2.set([]);
          this.proposalSlotsLoading2.set(false);
  ...
  private loadProposalSlots3(): void {
    ...
    this.profService
      .getAvailableSlots(apt.professionalId, this.proposalDate3(), durationMinutes)
      .subscribe({
        next: (slots) => {
          this.proposalSlots3.set(slots);
          this.proposalSlotsLoading3.set(false);
        },
        error: () => {
          this.proposalSlots3.set([]);
          this.proposalSlotsLoading3.set(false);
  ...
  proposalSlotClass(slot: TimeSlotResponse, suggested: boolean): string {
    ...
    if (selected)
      return `${base} border-indigo-600 bg-indigo-600 text-white dark:border-indigo-300 dark:bg-indigo-700 dark:text-white`;
    if (suggested)
      return `${base} border-blue-200 bg-white text-blue-900 hover:border-blue-400 hover:bg-blue-100 dark:border-blue-700 dark:bg-slate-900 dark:text-blue-100 dark:hover:border-blue-500 dark:hover:bg-blue-900/40`;
  ...
  private doAction(
    action: () => import('rxjs').Observable<AppointmentResponse>,
    onSuccess?: () => void,
  ): void {
    this.actionLoading.set(true);
    this.error.set('');
  ...
  statusLabel(status: AppointmentStatus): string {
    const map: Record<string, string> = {
      ...
      PROPOSED_NEW_TIME: 'Proposta orario',
  ...
  private loadProposalSlots(): void {
    ...
    this.proposalError.set('');
    this.profService
      .getAvailableSlots(apt.professionalId, this.proposalDate(), durationMinutes)
      .subscribe({
        next: (slots) => {
